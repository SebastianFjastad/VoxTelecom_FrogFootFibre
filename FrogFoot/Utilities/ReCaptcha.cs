...
namespace FrogFoot.Utilities
{
    ...
    public class ReCaptchaAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if (string.IsNullOrWhiteSpace(privateKey))
                throw new InvalidKeyException("ReCaptchaPrivateKey missing from appSettings");
    ...
    public static class HtmlHelperExtensions
    {
        ...
        {
            const string htmlInjectString = @"<div id=""recaptcha_div""></div><script type=""text/javascript"">var onloadCallback = function(){{grecaptcha.render(""{0}"", ""recaptcha_div"", {{ theme: ""{1}"" {2}}})}};</script>";

            var publicKey = ConfigurationManager.AppSettings["ReCaptcha.PublicKey"];

            if (string.IsNullOrWhiteSpace(publicKey))
                throw new InvalidKeyException("ReCaptchaPublicKey missing from appSettings");
